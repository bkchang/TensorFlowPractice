...
#    sess.run(train_step, feed_dict={x: batch_xs, y_: batch_ys, keep_prob: 1.0})
    _, train_summary = sess.run([train_step, merged_summary_op], feed_dict={x: batch_xs, y_: batch_ys})
    train_writer.add_summary(train_summary, step)
    test_summary = sess.run(merged_summary_op, feed_dict={x: mnist.test.images, y_: mnist.test.labels})
